...
Client.prototype._connect = function() {
  ...
  socket
    ...
    .on('end', function() {
      ...
      if (self.ending) {
        // @todo destroy()?
		this.log.info("MYSQLCLIENT - self.ending is true");
        self.connected = false;
        self.ending = false;

        if (self._queue.length) {
		  this.log.info("MYSQLCLIENT - call self _connect 1");
          self._connect();
        }	
        return;
      }
      ...
      if (!self.connected) {
	    this.log.info("MYSQLCLIENT - !self.connected ... emit error");
        this.emit('error', new Error('reconnection attempt failed before connection was fully set up'));
        return;
      }
	  this.log.info("MYSQLCLIENT - call self _connect 2");
      self._connect();
    })
