...
Client.prototype._connect = function() {
  ...
  socket
    ...
    .on('end', function() {
	  console.log("MYSQLCLIENT - received socket end");
      if (self.ending) {
        // @todo destroy()?
		console.log("MYSQLCLIENT - self.ending is true");
        self.connected = false;
        self.ending = false;

        if (self._queue.length) {
		  console.log("MYSQLCLIENT - call self _connect 1");
          self._connect();
        }	
        return;
      }
      ...
      if (!self.connected) {
	    console.log("MYSQLCLIENT - !self.connected ... emit error");
        this.emit('error', new Error('reconnection attempt failed before connection was fully set up'));
        return;
      }
	  console.log("MYSQLCLIENT - call self _connect 2");
      self._connect();
    })
...
Client.prototype.write = function(packet) {
  ...
  this._socket.write(packet.buffer , function ( err ) {
	if ( err ) {
		console.log("error in socket write callback : " + err );
	}
  });
};
...
Client.prototype._connectionErrorHandler = function() {
  return function(err) {
    console.log("MYSQLCLIENT - in _connectionErrorHandler");
    this.destroy();
    ...
    if (delegate instanceof Query) {
	  console.log("MYSQLCLIENT - call error on query");
      delegate.emit('error', err);
      return;
    }

    if (!delegate) {
	  console.log("MYSQLCLIENT - emit error on this object");
      this.emit('error', err);
    } else {
	  console.log("MYSQLCLIENT - callback error on delegate");
      delegate(err);
      this._queue.shift();
    }

    if (this._queue.length) {
	  console.log("MYSQLCLIENT - call self _connect from _connectionErrorHandler");
      this._connect();
    }
  }.bind(this);
};
