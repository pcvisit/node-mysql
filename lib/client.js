...
function Client(log) {
  if (!(this instanceof Client) || arguments.length > 1) {
...
Client.prototype._connect = function() {
  ...
  socket
    ...
    .on('end', function() {
	  this.log.info("MYSQLCLIENT - received socket end");
      if (self.ending) {
        // @todo destroy()?
        self.log.info("MYSQLCLIENT - self.ending is true");
        self.connected = false;
        self.ending = false;

        if (self._queue.length) {
          self.log.info("MYSQLCLIENT - call self _connect 1");
          self._connect();
        }	
        return;
      }
      ...
      if (!self.connected) {
        self.log.info("MYSQLCLIENT - !self.connected ... emit error");
        self.emit('error', new Error('reconnection attempt failed before connection was fully set up'));
        return;
      }
      self.log.info("MYSQLCLIENT - call self _connect 2");
      self._connect();
    })
...
Client.prototype.write = function(packet) {
  if (this.debug) {
    this.log.info('-> %s', packet.buffer.inspect());
  }

  this._socket.write(packet.buffer , function ( err ) {
	if ( err ) {
		this.log.info("error in socket write callback : " + err );
	}
  });
};
...
Client.prototype._connectionErrorHandler = function() {
  return function(err) {
    this.log.info("MYSQLCLIENT - in _connectionErrorHandler");
    this.destroy();
    ...
    if (delegate instanceof Query) {
	  this.log.info("MYSQLCLIENT - call error on query");
      delegate.emit('error', err);
      return;
    }

    if (!delegate) {
	  this.log.info("MYSQLCLIENT - emit error on this object");
      this.emit('error', err);
    } else {
	  this.log.info("MYSQLCLIENT - callback error on delegate");
      delegate(err);
      this._queue.shift();
    }

    if (this._queue.length) {
	  this.log.info("MYSQLCLIENT - call self _connect from _connectionErrorHandler");
      this._connect();
    }
  }.bind(this);
};
...
Client.prototype._debugPacket = function(packet) {
  ...
  this.log.info('<- %s: %j', packetName, packet);
};
